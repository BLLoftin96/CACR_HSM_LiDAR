...
# Father and Sons
# Filter High Noise
F_S_Camp_4_12_IVC <- classify_noise(F_S_Camp, ivf(res = 4, n = 12))

unique(F_S_Camp_4_12_IVC@data[["Classification"]]) # shows the number of unique classifications: 1 (unclassified), 2 (ground), 3 (low vegetation), 4 (medium vegetation), 5 (high vegetation), 7 (low noise), and 18 (high noise). 
prior_filter_FSCamp <- npoints(F_S_Camp_4_12_IVC) # number of points before filter - 34099924

F_S_Camp_412_Filter = filter_poi(F_S_Camp_4_12_IVC, Classification!= LASNOISE)
after_filter_FSCamp <- npoints(F_S_Camp_412_Filter) # number of points before filter - 34099379 - looks like ~545 points were removed -> less than 1%
lidRviewer::view(F_S_Camp_412_Filter) # visually inspect - looks pretty good with no obvious noise
lidRviewer::view(F_S_Camp)  # if you need to compare

# Filter low Noise

F_S_Camp_NoiseFilter = filter_poi(F_S_Camp_412_Filter, Classification!= 7L)
after_filter_FSCamp2 <- npoints(F_S_Camp_NoiseFilter) # number of points before filter - 34097223 
lidRviewer::view(F_S_Camp_NoiseFilter) # visually inspect - looks pretty good with no obvious noise

# Bear Gulch
# Classify all returns 
B_G_Camp_4_12_IVC <- classify_noise(B_G_Camp, ivf(res = 4, n = 12))

unique(B_G_Camp_4_12_IVC@data[["Classification"]]) # shows the number of unique classifications: 1 (unclassified), 2 (ground), 5 (high vegetation), 7 (low vegetation/low noise), and 18 (high noise). 
prior_filter_BGCamp <- npoints(B_G_Camp_4_12_IVC) # number of points before filter 52091492

B_G_Camp_412_Filter = filter_poi(B_G_Camp_4_12_IVC, Classification!= LASNOISE)
after_filter_BGCamp <- npoints(B_G_Camp_412_Filter) # number of points before filter - 52091413 - looks like ~79 points were removed
lidRviewer::view(B_G_Camp_412_Filter) # visually inspect - looks pretty good with no obvious noise
lidRviewer::view(B_G_Camp) # if you need to compare

# Filter low Noise

B_G_Camp_NoiseFilter = filter_poi(B_G_Camp_412_Filter, Classification!= 7L)
after_filter_BGCamp2 <- npoints(B_G_Camp_NoiseFilter) # number of points before filter -  52091279
lidRviewer::view(B_G_Camp_NoiseFilter) # visually inspect - looks pretty good with no obvious noise


# Thompson Flat
T_F_Camp_4_12_IVC <- classify_noise(T_F_Camp, ivf(res = 4, n = 12))

unique(T_F_Camp_4_12_IVC@data[["Classification"]]) # shows the number of unique classifications: 1 (unclassified), 2 (ground), 5 (high vegetation), 7 (low vegetation/low noise), and 18 (high noise). 
prior_filter_TFCamp <- npoints(T_F_Camp_4_12_IVC) # number of points before filter - 58340605

T_F_Camp_412_Filter = filter_poi(T_F_Camp_4_12_IVC, Classification!= LASNOISE)
after_filter_TFCamp <- npoints(T_F_Camp_412_Filter) # number of points before filter - 58339116 - looks like ~1489 points were removed -> less than 1%
lidRviewer::view(T_F_Camp_412_Filter) # visually inspect - looks pretty good with no obvious noise
lidRviewer::view(T_F_Camp) # if you need to compare

# Filter low Noise

T_F_Camp_NoiseFilter = filter_poi(T_F_Camp_412_Filter, Classification!= 7L)
after_filter_TFCamp2 <- npoints(T_F_Camp_NoiseFilter) # number of points before filter -  
lidRviewer::view(T_F_Camp_NoiseFilter) # visually inspect - looks pretty good with no obvious noise
```
