...
**Note: code chunks are not set to run to avoid issues with memory. Rather code chunks should only display code for reproducibility.**
Commands:
...
# Imagery based on lidRviewer looks similar to what I have seen, very dense point cloud. Visually, I could only see 1 or two points that are a little above the tree line - probably a bird. 
...
# lidRviewer shows 7 points that are way above the tree line. Points look very good and dense so I feel they look accurate to the ground.
...
# lidRviewer had maybe ~5 points that seemed off, but it looked very good.
```
...
Southern ID 2018 #12 appears to have 4 different providers using 2 different systems of LiDAR. Notably the Galaxy PRIME system had a max of 7 - 8 returns while the VQ1560i had unlimited returns. 
...
# liRviewer shows some points above the trees. Also notice some empty space below the canopy where it looks like the laser couldn't quite make it to the ground
...
# lidRviewer shows many points above the treeline. Very dense location but can see some delineation of canopy but a bit harder to tell in this program.
...
# lidRviewer shows many points that are above the treeline. This region is probably the densest for the area, but it looked good from the quick check.
```
