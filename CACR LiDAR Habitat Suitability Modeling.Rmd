...
Here I am selecting a few locations from each of the flight paths (e.g., Elko, South ID 2018 #12, etc.) around my study area to visually inspect to see if the point cloud even reflects reality. To do this, I am using a package called lidRviewer that works in tandem with the lidR package to allow you to visualize the point cloud of selected tiles. The view acts by opening a new window that you can interface with - while open R cannot run other sections of code. I will also use *Cloud Compare* - an open source software - as it provides a more detailed and user-controlled view. The reason we are doing this is that the flight reports indicate that multiple agencies were hired to perform the flights so I want to check point cloud density even though the accuracy was tested.
...
# Load the Tile 
Log_MLPF107_Elko = readLAS('R:/ElkoNW_LAZ/USGS_LPC_ID_SouthernID_2018_D19_11TQG350815.laz')
